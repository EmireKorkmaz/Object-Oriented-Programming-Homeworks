...
ConnectFour::ConnectFour(string file): filename(file){
    openFile();
    printAll();
}
ConnectFour::ConnectFour(): rowSize(5),colSize(5){
	gameBoard = new Cell*[rowSize];
        for (int i = 0; i < rowSize; ++i)
            gameBoard[i] = new Cell[colSize];
        deleteIt=true; // object is created before reading any file, to delete the array to create it again,
        				// deleteIt is a flag to let playGame delete the existed array before read any file
}
// copy constructor
...
bool ConnectFour::playGame(){
	if(deleteIt){ // if object is created before reading any file, deletes the array to create it again,
					// after reading file given file.
		for (int i = 0; i < rowSize; ++i)
            delete [] gameBoard[i];
        delete [] gameBoard;
        deleteIt=false;
	}
	if(!isFileOpened){ // in mode "M", to prevent playGame to ask for a filename again and again,
					// i created a variable named isFileOpened it is false at the first,
					// after a file is opened it is become true. i did this to make sure
					// that playGame won't ask for a filename over and over againf
