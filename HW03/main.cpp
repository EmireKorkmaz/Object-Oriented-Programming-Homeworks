/*
	...
	or against another opponent. And after all these, program asks two integers
...
int ConnectFour::temp=0;

int main(int argc, char const *argv[]){

    string gameMode;
    int exit=0, whoIsDone[NUM_OF_OBJECTS]; /* every object starts with 1, if they're ended then their value will be 0.
    ...
    if (gameMode=="M") {
        ...
        while (exit<NUM_OF_OBJECTS) { // if a game ends then it increases exit's value
            ...
            if (objects[0].getIsFilled()>4 && objects[1].getIsFilled()>4)
	            if(objects[0].compareObj(objects[1]))
	            	cout << "First object is more likely to win the game.\n";
	            else if(!objects[0].compareObj(objects[1]))
